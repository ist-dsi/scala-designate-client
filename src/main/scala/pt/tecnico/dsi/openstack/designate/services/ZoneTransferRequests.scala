...
final class ZoneTransferRequests[F[_]: Sync: Client](baseUri: Uri, authToken: Header, createUri: String => Uri) extends Service[F](authToken) {
  ...
  def list(query: Query, extraHeaders: Header*): Stream[F, WithId[ZoneTransferRequest]] = super.list[WithId[ZoneTransferRequest]]("transfer_requests", uri, query, extraHeaders:_*)
  ...
  def create(zoneId: String, value: ZoneTransferRequest.Create, extraHeaders: Header*): F[WithId[ZoneTransferRequest]] =
    POST.apply(value, createUri(zoneId) / "transfer_requests", (authToken +: extraHeaders):_*).flatMap(client.run(_).use {
      ...
      case Conflict(_) =>
        list(extraHeaders:_*).filter(_.zoneId == zoneId).head.compile.lastOrError.flatMap { existing =>
  ...
  def get(id: String, extraHeaders: Header*): F[WithId[ZoneTransferRequest]] = super.get(wrappedAt = None, uri / id, extraHeaders:_*)
